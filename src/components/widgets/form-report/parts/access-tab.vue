<template>
  ...
  <v-card flat>
    ...
    <v-switch
      :label="`${(form.config.access.enabled)?'was opened':'was closed'} ${(form.config.access.lastNotificatedAt)?timeAgo(form.config.access.lastNotificatedAt):''}`"
      v-model="form.config.access.enabled"
      @change="onUpdateAccess()"
      :disabled="!form.config.questions"
