<template>
    ...
    <v-flex xs12 md8>
      <v-input label="Chart Height">
        <div style="padding: 0 0 0 1em;">
          <input 
            type="number" 
            style="border: 1px solid #e0e0e0;" 
            v-model="config.options.widget.height"
            min=0
          ></input>
        </div>  
      </v-input>
...
import palettePicker from "@/components/core/ext/palette-picker.vue"
    ...
    methods:{
      onChangePalette(value){
        let color = JSON.parse(JSON.stringify(value)).color;
        if(value.isReverse) color.reverse();
        this.config.options.color = color
    ...
    created(){
    console.log("decoration CREATED", this.config)
    },
    mounted(){
      console.log("decoration MOUNTED", this.config)
    }
