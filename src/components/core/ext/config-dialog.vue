<template>
  <v-card>
    ...
    <v-layout>
      <v-flex>
        <v-card-text class="pa-1" style="height:500px;">
          <v-treeview :active.sync="active" :items="editorTree" :open="open" activatable active-class="primary--text" open-on-click transition>
          </v-treeview>
        </v-card-text>
      </v-flex>
    ...
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn color="primary" flat="flat" @click="reject">
        Cancel
      </v-btn>
      <v-btn color="primary" flat="flat" @click="resolve">
        Ok
      </v-btn>
    </v-card-actions>
  </v-card>
...
<script>
import widgetTypesPanel from "@/components/core/dialogs/widgetTypesPanel.vue"
...
let components = {
  "widget-types-panel": widgetTypesPanel
}
...
export default {
  ...
  created() {
    ...
    console.log("Config dialog created")
  ...
  mounted() {
    console.log("Conf dialog mounted", this.editorTree)
  }
